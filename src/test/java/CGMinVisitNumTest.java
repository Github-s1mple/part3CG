...
public class CGMinVisitNumTest {
    // 定义需要测试的多组 EXPAND_STEP 参数（可根据需求扩展）
    private static final List<Integer> TEST_PARAMS = new ArrayList<>();

    static {
        // 初始化测试参数组（示例：5组不同的步长组合）
        // 格式：每组6个值，对应6个阶段的步长
        TEST_PARAMS.add(0); // 原始参数
        TEST_PARAMS.add(1); // 步长偏小
        TEST_PARAMS.add(2); // 步长偏大
        TEST_PARAMS.add(3); // 统一步长
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int groupIdx = 0; groupIdx < TEST_PARAMS.size(); groupIdx++) {
            Integer currentStep = TEST_PARAMS.get(groupIdx);
            System.out.println("\n==================== 开始测试第 " + (groupIdx + 1) + " 组参数 ====================");
            System.out.println("当前EXPAND_STEP：" + currentStep);

            // 设置当前测试组的步长参数
            Constants.MIN_VISIT_NUM = currentStep;
            ...
            // 计算第一阶段实际步长（距离）
            double firstStageStep = 2 * Constants.MIN_VISIT_NUM;

            // 输出当前组结果
            System.out.println("第 " + (groupIdx + 1) + " 组测试完成：");
            System.out.println("最小访问：" + firstStageStep);
        ...
        // 4. 保存结果汇总文件
        try (FileOutputStream fos = new FileOutputStream("最小访问灵敏度测试汇总.xlsx")) {
            resultWorkbook.write(fos);
            System.out.println("\n所有测试完成！结果汇总文件已保存：最小访问灵敏度测试汇总.xlsx");
        } catch (IOException e) {
